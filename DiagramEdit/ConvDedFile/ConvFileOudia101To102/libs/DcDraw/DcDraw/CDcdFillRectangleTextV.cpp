...
CDcdFillRectangleTextV::CDcdFillRectangleTextV( 
	...
	, m_CDcdText( "" , CdDrawTextProp( aCdFontProp , colorrefText ) , strExtent)
...
bool CDcdFillRectangleTextV::DcDraw( IfDcdTarget* pIfDcdTarget ) 
{
	...
	Ou<CGdiHFontHolder>	pFontVertical ;
	{
		...
		if ( pFontVertical != NULL )
		{
			...
			{
				pFontVertical = Ou<CGdiHFontHolder>() ;
			}
		}
	}
	...
	int iXOffset = 0 ;
	{
		iLines = ( (int)ariBidx.size() - 1 + m_iLinesExtentY - 1 ) / m_iLinesExtentY ;
		iXOffset = 
	...
	for ( int idxChar = 0 ; idxChar < (int)ariBidx.size() - 1 ; idxChar ++ ){
		...
		if ( aCStrCharSizeMb.CsizeOfBidx( 0 ) > 1 && pFontVertical != NULL ){
			// --------------------------------
			//	全角文字（１文字）
			// --------------------------------
			CConverter_WinGdi	aConv ;

			HFONT hfontOld = (HFONT)SelectObject( pIfDcdTarget->getHdc() , pFontVertical->getHFont() ) ;
