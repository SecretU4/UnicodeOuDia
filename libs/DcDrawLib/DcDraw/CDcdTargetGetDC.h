 ...
 * @brief
 * 【概要】
 *	ウインドウのクライアント領域の、描画領域を提供するクラスです。
 *	指定されたウインドウ(HWND)のクライアント領域を Win32 GetDC() APIで取得し、
 *	ウインドウのクライアント領域全体を描画領域とします。
 *
 *	このクラスは、WM_PAINTメッセージが到着したとき以外の局面で使います。
 *	インスタンスは、スタック上に生成することを想定しています。
 *
 * 【使い方】
 *
 * １． スタック上にインスタンスを生成してください。
 *	コンストラクタでは、対象となるウインドウの HWND を指定してください。
 *
...
// ********************************
 private:
	/**
	 * 	対象となるウインドウハンドル
	 *	このHWNDの破棄の責務は クラスユーザーにあります。
	 * 	このHWNDは、thisより長く生存しなくてはなりません。
	 *
	 *	NULLでもかまいません。
	 *	NULL を指定すると、画面全体を表す
	 *	デバイスコンテキストが描画領域となります。
	 */
	HWND			m_hWnd ;
// --------------------------------
///@name	内部データ
...
///@{
	...
	/**
	 * 	ウインドウのクライアント領域のDC(デバイスコンテキスト)。
	 *	このDCの生成・破棄の責務は this にあります。
	 * 	this は、このDCを コンストラクタで生成し、デストラクタで破棄します。
	 */
	HDC			m_hDc ;
