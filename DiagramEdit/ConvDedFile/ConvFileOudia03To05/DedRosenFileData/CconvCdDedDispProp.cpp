			...
			OuPropertiesText::CNodeContainer* pCNodeContainer ) 
{
	...
	if ( iRv >= 0 )
	{
		...
		pCNodeContainer->setValue( nameJikokuhyouFont , strValue ) ;
	}
	//2.DiaEkimeiFont
	if ( iRv >= 0 ){
	...
	{
		//	デフォルト値の場合は、出力はしません。
		if ( aCdDedDispProp.getJikokuhyouRessyaWidth() != 
			CdDedDispProp().getJikokuhyouRessyaWidth() )
		{
	...
	//12.DiaRessyajouhouHyoujiEkiOrderKudari
	if ( iRv >= 0 ){
		int iValue = aCdDedDispProp.getDiaRessyajouhouHyoujiEkiOrder( Ressyahoukou_Kudari ) ;
		char	szLs[32] ;
		sprintf( szLs , "%d" , iValue ) ;
		pCNodeContainer->setValue( nameDiaRessyajouhouHyoujiEkiOrderKudari , szLs  ) ;
	}
	//13.DiaRessyajouhouHyoujiEkiOrderNobori
	if ( iRv >= 0 ){
		int iValue = aCdDedDispProp.getDiaRessyajouhouHyoujiEkiOrder( Ressyahoukou_Nobori ) ;
		char	szLs[32] ;
		sprintf( szLs , "%d" , iValue ) ;
		pCNodeContainer->setValue( nameDiaRessyajouhouHyoujiEkiOrderNobori , szLs  ) ;
	}
			...
			OuPropertiesText::ErrorInfo::COuErrorInfoContainer* pCOuErrorInfoContainer ) 
{
	...
	//1.JikokuhyouFont
	if ( iRv >= 0 ){
		string	strValue = pCNodeContainer->getValue( nameJikokuhyouFont ) ;
	...
	//12.DiaRessyajouhouHyoujiEkiOrderKudari
	if ( iRv >= 0 ){
		string	strValue = pCNodeContainer->getValue( nameDiaRessyajouhouHyoujiEkiOrderKudari ) ;
		int iValue = atoi( strValue.c_str() ) ;
		if ( !( 0 <= iValue ) ){
				COuErrorInfo aCOuErrorInfo( ERRREASON_InvalidValue() ) ;
				aCOuErrorInfo.getProp()->setValue( ERRPROP_Name() , nameDiaRessyajouhouHyoujiEkiOrderKudari ) ; 
				aCOuErrorInfo.getProp()->setValue( ERRPROP_Value() , strValue ) ; 
	...
	//13.DiaRessyajouhouHyoujiEkiOrderNobori
	if ( iRv >= 0 ){
		string	strValue = pCNodeContainer->getValue( nameDiaRessyajouhouHyoujiEkiOrderNobori ) ;
		int iValue = atoi( strValue.c_str() ) ;
		if ( !( 0 <= iValue ) ){
				COuErrorInfo aCOuErrorInfo( ERRREASON_InvalidValue() ) ;
				aCOuErrorInfo.getProp()->setValue( ERRPROP_Name() , nameDiaRessyajouhouHyoujiEkiOrderNobori ) ; 
				aCOuErrorInfo.getProp()->setValue( ERRPROP_Value() , strValue ) ; 
