...
*/
//$Id: OuPropertiesText.h 231 2016-01-05 05:28:24Z okm $
/**
 @file 
 @brief
	OuLib::Str::OuPropertiesText 
...
/** 
@brief
	OuPropertiesText を扱うクラス群です。
	
	OuPropertiesText は、キー名=値 の形式でデータを保存することのできる
	テキストベースのデータフォーマットです。

	OuPropertiesText のデータ(ノード)は、ツリー形式で階層化することが
	できます。一つのノードは、通常は異なる名前の子ノードを持ちますが、同一名の
	子ノードを複数持たせることも可能です。
	
	OuPropertiesTextは、 .ini ファイル形式に対して、2段階以上のセクション階層
	を記述できるような拡張を施したフォーマットであると言えます。
*/
...
#include "Str\OuPropertiesText\ErrorInfo\ErrorInfo.h"
