...
string CdDedJikoku::CConv::encode( const CdDedJikoku& aJikoku )const 
{
	...
	if ( !aJikoku.isNull() ){
		...
		if ( m_eHour == EHour_ZeroToNone )
		{
			...
			//strRv += szLs ;
			strRv += OuLib::strprintf( "%d" , aJikoku.getHour() );
		} 
		else if ( m_eHour == EHour_ZeroToSpace )
		{
			...
			//strRv += szLs ;
			strRv += OuLib::strprintf( "%2d" , aJikoku.getHour() );
		}
		else
		{
			//char	szLs[16] ;
			...
			//strRv += szLs ;
			strRv += OuLib::strprintf( "%02d" , aJikoku.getHour() );
		}
		//	時のつぎのコロン
		if ( !m_bNoColon )
		{
		...
		//	分
		{
			strRv += OuLib::strprintf( "%02d" , aJikoku.getMinute()  );
		}
		//	秒
			...
			( m_eSecond == ESecond_NotIfZero && aJikoku.getSecond() != 0 ) )
		{
			...
			{
				strRv += OuLib::strprintf( "%02d" , aJikoku.getSecond() );
			}
		}
	}
	return ( strRv ) ;
