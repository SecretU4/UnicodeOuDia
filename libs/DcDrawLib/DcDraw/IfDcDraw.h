 ...
 * @brief
 * 【概要】
 * 	  DcDrawモデルにおける、描画処理を行うクラスがサポートするインターフェース
 * 	です。
 *
 *	  このクラスが描画を行う対象（DC）は、IfDcdTarget インターフェースを
 *	サポートするオブジェクトで表します。
 *
 * 	  たとえば、ウインドウにWM_PAINTメッセージが到着したら、ウインドウは、
 * 	描画先のDCを表す CdDcdTargetOnPaint を生成し、その IfDcdTarget 
 *	インターフェースを指定して IfDcDraw::DcDraw() メソッドを呼び出します。
 * 	  DcDraw() メソッドが呼び出されると、 IfDcDraw オブジェクトは、渡された
 *	IfDcdTarget オブジェクトが保持するDCに、描画を行います。
 */
class IfDcDraw
{
 public:
