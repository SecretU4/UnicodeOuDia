...
#include <sstream>
using namespace std ;
static string logCStrCharSizeMb( 
	const string& aStr , 
	const CStrCharSizeMb& aStrCharSize )
{

	stringstream strm ;
	{
		strm << aStrCharSize.ByteLength() << endl ;
		for ( int idx = 0 ; idx < aStrCharSize.ByteLength() ; idx ++ ){
			strm << aStrCharSize.CsizeOfBidx( idx ) << " " ;
		}
		strm << endl ;
	}
	{
		strm << aStrCharSize.CharLength() << endl ;
		for ( int idx = 0 ; idx < aStrCharSize.CharLength() ; idx ++ ){
			strm << aStrCharSize.CsizeOfCidx( idx ) << " " ;
		}
		strm << endl ;
	}
	{
		for ( int idx = 0 ; idx < aStrCharSize.ByteLength() ; ){
			strm << aStr.substr( idx , aStrCharSize.CsizeOfBidx( idx ) ) << " " ;
...
CStrCharSizeMb& CStrCharSizeMb::scan( const std::string& ev ) 
{
	...
	/*debug
	{
		LogMsgL( 3 , "CStrCharSizeMb::scan\n%s" , 
			logCStrCharSizeMb( ev , *this ).c_str() ) ;
	}
	*/
