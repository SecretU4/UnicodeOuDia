...
int CentDedEkiCont::JikokuhyouIndexOfJikokuOrder( 
		ERessyahoukou eRessyahoukou , 
		int iJikokuOrder  )const
{
	//	JikokuOrderが決定したら、JikokuOrderが入ります。
	int iJikokuhyouIndex = -1 ;	

	const IfContGet<CentDedEkiCont::CdDedEki>* pCdDedEkiCont = 
		getIfContGet( eRessyahoukou ) ;

	if ( iJikokuOrder == INT_MAX )
	{
		iJikokuOrder = pCdDedEkiCont->size() * 2 ;
	}
	if ( !( 0 <= iJikokuOrder && iJikokuOrder <= INT_MAX ) )
	{
		return -1 ;
	}
	else	
	{
		int iJikokuhyouIndexCurr = 0 ;
		int iJikokuOrderCurr = 0 ; 
		for ( iJikokuOrderCurr = 0 ; 
			iJikokuOrderCurr < iJikokuOrder ; iJikokuOrderCurr ++ )
		{
			int iEkiOrderCurr = iJikokuOrderCurr / 2 ;
			int iHatsu = iJikokuOrderCurr % 2 ;
			CdDedEki aCdDedEki = pCdDedEkiCont->get( iEkiOrderCurr ) ;
			if ( iHatsu == 0 && aCdDedEki.getChakujikokuHyouji() )
			{
				iJikokuhyouIndexCurr ++ ;
			}
			if ( iHatsu == 1 && aCdDedEki.getHatsujikokuHyouji() )
			{
				iJikokuhyouIndexCurr ++ ;
			}
		}
		//iJikokuhyouIndexCurr = 指定されたJikokuOrder 以前に表示される時刻の数

		if ( iJikokuOrder == pCdDedEkiCont->size() * 2 )
		{
			//	時刻表列数すべてを取得します。
			iJikokuhyouIndex = iJikokuhyouIndexCurr ;
		}
		else
		{
			int iEkiOrderCurr = iJikokuOrderCurr / 2 ;
			...
			else if ( iHatsu == 1 && aCdDedEki.getHatsujikokuHyouji() )
			{
				iJikokuhyouIndex = iJikokuhyouIndexCurr ;
			}
		}
	...
	return iJikokuhyouIndex ;

}

int CentDedEkiCont::JikokuOrderOfJikokuhyouIndex( 
		ERessyahoukou eRessyahoukou , 
		int iJikokuhyouIndex  )const 
{
	...
	for ( iJikokuOrderCurr = 0 ; 
		iJikokuOrderCurr < pCdDedEkiCont->size() * 2  ; 
		iJikokuOrderCurr ++ )
	{
		int iEkiOrderCurr = iJikokuOrderCurr / 2 ;
		int iHatsu = iJikokuOrderCurr % 2 ;
		...
		if ( iHatsu == 0 && aCdDedEki.getChakujikokuHyouji() )
		{
			if ( iJikokuhyouIndexCurr == iJikokuhyouIndex )
			{
				iJikokuOrder = iJikokuOrderCurr ;
				break ;
			}
			iJikokuhyouIndexCurr ++ ;
		}
