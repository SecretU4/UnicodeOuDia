...
unsigned int CdDedJikan::m_iFmt = 0  ;
...
string CdDedJikan::encode( unsigned int iFmt ) const
{
	string	strRv ;
	{
		//  符号
		if ( ( iFmt & Format_PlusDisplay ) == Format_PlusDisplay ){
				strRv += "-++"[ sign( m_iTotalSeconds ) + 1 ] ;
		} else if ( ( iFmt & Format_PlusToSpace ) == Format_PlusToSpace ){
				strRv += "-  "[ sign( m_iTotalSeconds ) + 1 ] ;
		}	else	{
