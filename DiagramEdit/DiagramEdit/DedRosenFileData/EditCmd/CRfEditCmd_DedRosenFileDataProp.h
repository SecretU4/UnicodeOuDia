...
/**
 * @brief
  CDedRosenFileData オブジェクトの属性
『路線ファイルのプロパティ』ダイアログで更新できる属性
を上書きする編集動作です。
...
private:
	...
	/**
		...
		既定値は 60 です。
		コンストラクタで初期化されます。この値は、ダイアログ表示時に
		初期値として使われます。
		OnOK() は、ダイアログの値で上書きします。クラスユーザーは、
		 DoModal() がリターンした後に、この値を参照することができます。
	*/
...
public:
	// ********************************
	///@name CRfEditCmd_DedRosenFileDataProp-属性
	// ********************************
	///@{
