...
　1.　インスタンスを生成してください。
 コンストラクタ引数には、DLLファイルのファイル名を指定してください。

　2.　validate() を呼び出して、インスタンスを有効化してください。
 　validate()メソッドは、ConvFileDll をロードします。

  3.  getIfConvFile() で、ConvFileDll の IfConvFile インターフェースを
取得することができます。IfConvFileメソッドを使って、ファイルフォーマットの
変換を行うことができます。
