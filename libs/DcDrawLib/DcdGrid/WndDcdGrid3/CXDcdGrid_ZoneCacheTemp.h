...
/**
@brief
	WndDcdGrid3::CXDcdGrid に対して ZoneCache を有効にします。
	これにより、WndDcdGrid3::CXDcdGrid から 座標位置・サイズの情報を
	高速取得できるようにします。
*/
...
public:
	...
	// ********************************
	///@{
	/**	このオブジェクトは、関連が有効な間は存在しなくてはなりません。	*/
...
private:
	...
	///@{
	/**
		true:コンストラクタでキャッシュを新しく作成しました。
			デストラクタではキャッシュを削除しなくてはなりません。
	*/
