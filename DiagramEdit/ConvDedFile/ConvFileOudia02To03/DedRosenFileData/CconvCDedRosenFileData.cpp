...
int CconvCDedRosenFileData::CDedRosenFileData_to_CdConnectedString( 
		...
		CdConnectedString2* pCdConnectedString ) 
{
	...
	if ( iRv >= 0 ){
		...
		int iResult = aCconvCdDedDispProp.toCdConnectedString( 
...
int CconvCDedRosenFileData::CDedRosenFileData_from_CdConnectedString( 
		...
		const CdConnectedString2& aCdConnectedString ) 
{
	...
	if ( iRv >= 0 ){
		...
		int iResult = aCconvCdDedDispProp.fromCdConnectedString( 
			&aCdDedDispProp ,
			connectstrDispProp ) ;
		if ( iResult < 0 ){
			iRv = -21 ;	//	DispProp の内容が不正
