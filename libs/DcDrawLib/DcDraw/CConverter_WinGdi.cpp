...
void CConverter_WinGdi::toRECT( const CdDcdZoneXy& aCdDcdZoneXy , RECT* pRECT ) 
{
	...
	//	@name Colorを扱う関数群
	// ********************************
CdColorProp CConverter_WinGdi::CdColorPropOf( COLORREF value )
{
	return CdColorProp( GetRValue( value ) , GetGValue( value ) , GetBValue( value ) ) ;
}

COLORREF CConverter_WinGdi::COLORREFOf( const CdColorProp& value )
{
			...
			HDC hDc  ) 
{
	...
	if ( aCdFontProp.getPointTextHeight() > 0 )
	{
		...
		CdDcdSizeXy aSize = DPtoLP( CdDcdSizeXy( 0 , iDp ) ,hDc ) ;
		...
		HDC hDc  )
{
	...
	HFONT	hFont = NULL ;
	if ( iRv >= 0 ){
		hFont = CreateFont( aCdDrawTextProp.getCdFontProp() , 
									hDc ) ;
		if ( hFont == NULL ){
	...
	if ( iRv >= 0 ){
		...
		//--------------------------------
		{
			...
			CdWinGdi_DrawText_Format	aFlag( 
				CdWinGdi_DrawText_FormatOf( aCdDrawTextProp.getCdDrawTextFormat() ) ) ;
...
{
	CdWinGdi_DrawText_Format aRv ;
	switch( aCdDrawTextFormat.getHorizontal() )
	{
	case aCdDrawTextFormat.H_LEFT :	
		aRv.setHorizontal( aRv.H_LEFT ) ;
		break ;
	case aCdDrawTextFormat.H_CENTER :	
		aRv.setHorizontal( aRv.H_CENTER ) ;
		break ;
	case aCdDrawTextFormat.H_RIGHT :	
		aRv.setHorizontal( aRv.H_RIGHT ) ;
		break ;
	}

	switch( aCdDrawTextFormat.getVertical() )
	{
	case aCdDrawTextFormat.V_SINGLELINT_TOP :	
		aRv.setVertical( aRv.V_SINGLELINE_TOP ) ;
		break ;
	case aCdDrawTextFormat.V_SINGLELINT_CENTER :	
		aRv.setVertical( aRv.V_SINGLELINE_CENTER ) ;
		break ;
	case aCdDrawTextFormat.V_SINGLELINT_BOTTOM :	
		aRv.setVertical( aRv.V_SINGLELINE_BOTTOM ) ;
		break ;
	case aCdDrawTextFormat.V_MULTILINE :	
		aRv.setVertical( aRv.V_MULTILINE ) ;
		break ;
	case aCdDrawTextFormat.V_MULTILINE_WORDBREAK :	
		aRv.setVertical( aRv.V_MULTILINE_WORDBREAK ) ;
		break ;
	case aCdDrawTextFormat.V_NOFULLWIDTHCHARBREAK :	
		aRv.setVertical( aRv.V_NOFULLWIDTHCHARBREAK ) ;
		break ;
	}

	switch( aCdDrawTextFormat.getEllipses() )
	{
	case aCdDrawTextFormat.NOELLIPSIS :	
		aRv.setEllipses( aRv.NOELLIPSIS ) ;
		break ;
	case aCdDrawTextFormat.END_ELLIPSIS :	
		aRv.setEllipses( aRv.END_ELLIPSIS ) ;
		break ;
	case aCdDrawTextFormat.PATH_ELLIPSIS :	
		aRv.setEllipses( aRv.PATH_ELLIPSIS ) ;
		break ;
	case aCdDrawTextFormat.WORD_ELLIPSIS :	
		aRv.setEllipses( aRv.WORD_ELLIPSIS ) ;
		break ;
	}
	switch( aCdDrawTextFormat.getReading() )
	{
	case aCdDrawTextFormat.R_LEFT :	
		aRv.setReading( aRv.R_LEFT ) ;
		break ;
	case aCdDrawTextFormat.R_RIGHT :	
		aRv.setReading( aRv.R_RIGHT ) ;
		break ;
	}

	switch( aCdDrawTextFormat.getAmpresand() )
	{
	case aCdDrawTextFormat.EAmpersand_NoPrefix :	
		aRv.setPrefix( aRv.NOPREFIX ) ;
		break ;
	case aCdDrawTextFormat.EAmpersand_NormalPrefix :	
		aRv.setPrefix( aRv.NORMAL_PREFIX ) ;
		break ;
	}
	return aRv ;
}
