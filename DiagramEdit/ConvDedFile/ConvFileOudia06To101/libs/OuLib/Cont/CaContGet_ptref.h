 ...
 *	操作対象の "IfContGet< PtrType >"  インターフェースを、
 *	値・または参照型を要素とする "IfContGet< ValueType > 
 *	インターフェースに変換します。
 * 
 ...
 *  IfContGet< CBase* > インターフェースをサポートしたコンテナを
 * 他のオブジェクトに操作させる場合、他のオブジェクトには
 *  "IfContGet< const CBase& >" を渡したい。
 * 
@code
	CaCont_vector< vector< CBase* > >	aCaCont_vector_ptr ;
	CaContGet_ptref< const CBase& , CBase* > aCaContGet_ptref( &aCaCont_vector_ptr ) ;
	//aCaContGet_ptref = IfContGet< const CBase& > をサポートしています。
	//	このインターフェースにより、aCaCont_vector_ptr が保持している
 ...
 *	このクラスは、 "IfContGet< DataType1 >" インターフェースを
 *	サポートします。
 * @param PtrType
