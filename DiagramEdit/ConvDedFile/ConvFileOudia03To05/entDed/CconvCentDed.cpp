			...
			OuPropertiesText::CNodeContainer* pCNodeContainer ) 
{
	...
	if ( iRv >= 0 )
	{
		CdFontProp	aCdFontProp = aCentDedRessyasyubetsu.getJikokuhyouFont() ;
		if ( !aCdFontProp.isNull() )
		{
			CconvDcDrawProp	aConv ;
			pCNodeContainer->setValue( 
				nameJikokuhyouFont , 
				aConv.CdFontProp_to_string( aCdFontProp ) ) ;
		}
	}
	...
	if ( iRv >= 0 )
	{
		string strValue = getESenStyleToString().stringOf( aCentDedRessyasyubetsu.getDiagramSenStyle() ) ;
	...
	if ( iRv >= 0 )
	{
		CdFontProp	aCdFontProp = aCentDedRessyasyubetsu.getDiagramRessyaFont() ;
		if ( !aCdFontProp.isNull() )
		{
			CconvDcDrawProp	aConv ;
			pCNodeContainer->setValue( 
				nameDiagramRessyaFont , 
				aConv.CdFontProp_to_string( aCdFontProp ) ) ;
		}
	}
			...
			OuPropertiesText::ErrorInfo::COuErrorInfoContainer* pCOuErrorInfoContainer ) 
{
	...
	if ( iRv >= 0 )
	{
		...
		if ( !strValue.empty() )
		{
			...
			int iResult = aConv.CdFontProp_from_string( 
				&aCdFontProp , strValue ) ;
			if ( iResult < 0 )
			{
			...
			{
				aCentDedRessyasyubetsu.setJikokuhyouFont( aCdFontProp ) ;
			}
		}
	}
	...
	if ( iRv >= 0 )
	{
		...
		{
			aCentDedRessyasyubetsu.setDiagramSenStyle( eValue ) ;
		}
	}
	...
	if ( iRv >= 0 )
	{
		...
		if ( !strValue.empty() )
		{
			...
			{
				COuErrorInfo	aCOuErrorInfo( ERRREASON_InvalidValueFormat() ) ;
				aCOuErrorInfo.getProp()->setValue( ERRPROP_Name() , nameDiagramRessyaFont ) ;
				aCOuErrorInfo.getProp()->setValue( ERRPROP_Value() , strValue ) ;
				pCOuErrorInfoContainer->insert( aCOuErrorInfo ) ;
				iRv = -101 ;	//	値の形式が正しくありません。
			}
			...
			{
				aCentDedRessyasyubetsu.setDiagramRessyaFont( aCdFontProp ) ;
			}
		}
	}
			...
			OuPropertiesText::ErrorInfo::COuErrorInfoContainer* pCOuErrorInfoContainer ) 
{
	...
	if ( iRv >= 0 )
	{
		...
		else
		{
			...
			CaCont<CentDedEki>	aCaCont( pCentDedRosen->getCentDedEkiCont() ) ; 
	...
	if ( iRv >= 0 )
	{
		...
		else
		{
			...
			CaCont<CentDedRessyasyubetsu>	aCaCont( pCentDedRosen->getCentDedRessyasyubetsuCont() ) ; 
	...
	if ( iRv >= 0 )
	{
		...
		else
		{
			...
			CaCont<CentDedDia>	aCaCont( pCentDedRosen->getCentDedDiaCont() ) ; 
