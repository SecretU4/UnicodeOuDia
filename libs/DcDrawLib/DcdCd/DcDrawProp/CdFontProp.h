...
/**
 ...
 【概要】
  ...
  m_iPointTextHeight , m_iLogicalunitTextHeight , m_iLogicalunitCellHeight 
がすべて 0 なら、この属性は『NULL状態』とみなされます。
...
public:
	 ...
	 *	通常は偽、打ち消し線あれば真です。	
	 */
	CdFontProp( int iPointTextHeight , const std::string& strFacename ,
...
// ********************************
 public:
	...
	std::string	getFacename()const{	return m_strFacename ;	};
	CdFontProp& setFacename( const std::string& ev ){
		m_strFacename = ev ;	return *this ; } ;
	
	/**
	@return
		フォントの書体が縦書き対応である場合はtrue です。

		縦書き対応は、先頭1文字が '@@' であるか否かで判断します。
	*/
	bool isVerticalFacename()const
	{	return m_strFacename.length() >= 1 && m_strFacename[0] == '@' ;}

	/**
		フォントの書体を、横書き対応から縦書き対応に変更します。
		縦書き対応から横書き対応に変更することもできます。

		これは、書体名の先頭1文字の '@@' の有無を操作します。
	@param value [in]
		- true :  書体名が横書き対応なら、縦書き対応に変更します。
		- false :  書体名が縦書き対応なら、横書き対応に変更します。
	*/
	CdFontProp&  setVerticalFacename( bool value )
	{
		if ( value && !isVerticalFacename() ){	m_strFacename.insert( 0 , "@@" ) ; } 
		else if ( !value && isVerticalFacename() ){	m_strFacename.erase( 0 , 1 ) ; }
		return *this ;
	}
	...
	bool	getUnderline()const{
		return m_bUnderline ;	};
	CdFontProp& setUnderline( bool ev ){
		m_bUnderline = ev ;	return *this ; } ;
	...
	bool isEqualTo( const CdFontProp& value )const{
		return ( 
			...
			m_bUnderline == value.m_bUnderline && 
	...
	bool  less( const CdFontProp& b )const{
			...
			}	else if ( this->m_bUnderline != b.m_bUnderline ){
				return ( this->m_bUnderline < b.m_bUnderline ) ;
